 ...
 *
 * Admin credentials:
 * Email: admin@jobs-in-istanbul.com
...
async function seed() {
  ...
  try {
    ...
    const existing = await env.DB.prepare('SELECT id FROM auth_user WHERE email = ?').bind('admin@jobs-in-istanbul.com').first()
    if (existing) {
    ...
    await env.DB.batch([
      ...
      ).bind(odid, 'admin@jobs-in-istanbul.com', 'Admin', 'User', 'admin', 1, nowMs, nowMs, 'Admin User'),
      env.DB.prepare(
    ...
    console.log(`  Email: admin@jobs-in-istanbul.com`)
