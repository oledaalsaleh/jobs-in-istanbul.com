...
const dir = path.join(__dirname, '../public/images/istanbul-app');
...
console.log(`Found ${files.length} Jobs in Istanbul images to upload to R2...`);

for (const file of files) {
  const filePath = path.join(dir, file);
  const ext = path.extname(file).toLowerCase();
  const contentType = ext === '.png' ? 'image/png' : 'image/jpeg';
  const r2Key = `public/images/istanbul-app/${file}`;
...
console.log('Finished Jobs in Istanbul image uploads to R2.');
