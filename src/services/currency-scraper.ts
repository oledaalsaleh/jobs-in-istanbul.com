...
`;
  ...
  if (env.CACHE_KV) {
    try {
      await env.CACHE_KV.put('kv_currency_prices', JSON.stringify(dataObj));
      console.log('✓ Successfully stored currency prices in CACHE_KV.');
    } catch (kvErr) {
      console.warn('Failed to store currency prices in CACHE_KV:', kvErr);
    }
  }
  ...
  if (env.DB) {
    try {
      ...
      console.warn('Failed to persist currency prices to D1 DB (will rely on CACHE_KV):', dbErr);
    }
  }
